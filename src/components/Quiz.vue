            ...
            <div class="question-box">
                <div v-if="idx < count">
                    ...
                    <label v-for="(answer, index) in questions[idx].answers" 
                           :key="index" :for="index"
                           :class="(index == questions[idx].correctAnswer && selectedAnswer != ''), (selectedAnswer == index)">
                        <input :id="index" type="checkbox" class="hidden" :value="index" @change="answered($event)"
                            :disabled="selectedAnswer != ''">
                        {{answer}}
                    </label>
                        <button class="next" @click="nextQuestion" v-show="selectedAnswer != '' && idx < count - 1">
                            Next &gt;
                        </button>
                        <button class="finish" @click="showResults" v-show="selectedAnswer != '' && idx == count - 1">
                            Finish
                        </button>
                </div>
                <div v-else>
                    <h2>Results</h2>
                        <p>Correct Answers: <span>{{correctAnswers}}</span>
                        </p>
        ...
        return {
            ...
            questions: [
                ...
                },
                {
                    ...
                    answers: { a: 'Tim Berners-Lee', b: 'Brendan Eich', c: 'Bjarne Jacobsen', d: 'Håkon Wium Lie' },
                    correctAnswer: 'd'
                },
            ]
        }
